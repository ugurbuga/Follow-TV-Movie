...
abstract class BaseVmDbActivity<VM : BaseViewModel, DB : ViewDataBinding> :
    ...
    private val networkUtil: NetworkUtil by lazy { NetworkUtil() }
    ...
    private fun checkNetwork() {
        networkUtil.onChange = {
            if (!it) {
                showErrorDialog(R.string.no_internet)
            }
        }
    }
    ...
    public override fun onResume() {
        super.onResume()
        networkUtil.checkNetworkConnectivity(applicationContext)
        localizationDelegate.onResume(this)
    }

    override fun onPause() {
        networkUtil.removeRegister()
        super.onPause()
    }
